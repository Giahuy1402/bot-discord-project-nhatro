...
COLOR_PRIMARY = discord.Color.from_rgb(52, 152, 219)     # Sky Blue
COLOR_SUCCESS = discord.Color.from_rgb(46, 204, 113)     # Emerald Green
COLOR_WARNING = discord.Color.from_rgb(241, 196, 15)     # Sun Yellow
...
def get_payment_info() -> Dict[str, str]:
    ...
    return {
        ...
        "bank_account": os.environ.get("BANK_ACCOUNT", "0968999999"),
        "bank_owner": os.environ.get("BANK_OWNER", "GIA HUY"),
    }
...
def build_support_instructions() -> discord.Embed:
    ...
    return build_base_embed(title=f"💬 Hướng dẫn Liên hệ Hỗ trợ", description=desc, color=COLOR_INFO)
...
class EphemeralLinkView(discord.ui.View):
    ...
    async def click_link(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(LinkRoomModal())

class LinkRoomModal(discord.ui.Modal, title="Xác minh liên kết phòng trọ"):
    room_code = discord.ui.TextInput(
        label="Mã phòng trọ", 
        placeholder="Nhập mã phòng trọ (Ví dụ: P101, P202)",
        min_length=2, 
        max_length=20
    )
    link_code = discord.ui.TextInput(
        label="Mã liên kết xác minh", 
        placeholder="Nhập mã xác minh 8 ký tự lấy trên Desktop",
        min_length=8, 
        max_length=8
    )
...
class SupportMessageModal(discord.ui.Modal, title="Gửi tin nhắn liên hệ chủ trọ"):
    message_content = discord.ui.TextInput(
        ...
        placeholder="Nhập nội dung cần hỗ trợ gửi tới chủ trọ...",
        min_length=5,
        max_length=500
    )
...
class MenuView(discord.ui.View):
    ...
    @discord.ui.button(label="⚡ Điện nước", style=discord.ButtonStyle.secondary, row=1)
    async def btn_utilities(self, interaction: discord.Interaction, button: discord.ui.Button):
        room = await self.get_room(interaction)
        if room:
            embed = build_utilities_embed(room)
    ...
    @discord.ui.button(label="💬 Liên hệ chủ trọ", style=discord.ButtonStyle.primary, row=2)
    async def btn_support(self, interaction: discord.Interaction, button: discord.ui.Button):
        room = await self.get_room(interaction)
        if room:
            await interaction.response.send_modal(SupportMessageModal(room))

    @discord.ui.button(label="👤 Hồ sơ", style=discord.ButtonStyle.secondary, row=2)
    async def btn_profile(self, interaction: discord.Interaction, button: discord.ui.Button):
        room = await self.get_room(interaction)
        if room:
            embed = build_profile_embed(room)
...
class BotCommands(commands.Cog):
    ...
    @app_commands.command(name="utilities", description="Xem chỉ số điện nước tiêu thụ tháng mới nhất")
    async def slash_utilities(self, interaction: discord.Interaction):
        room = await self.get_linked_room(interaction)
        if room:
            embed = build_utilities_embed(room)
            await interaction.response.send_message(embed=embed, ephemeral=True)

    @app_commands.command(name="bank", description="Lấy thông tin tài khoản chuyển khoản ngân hàng và mã QR VietQR")
    ...
    @app_commands.command(name="support", description="Gửi tin nhắn phản hồi trực tiếp cho chủ trọ")
    @app_commands.describe(noidung="Nội dung tin nhắn muốn gửi tới phần mềm của chủ trọ")
    async def slash_support(self, interaction: discord.Interaction, noidung: str):
        room = database.get_room_by_discord_id(interaction.user.id)
        if not room:
            embed = build_base_embed(
                title="❌ Tài khoản Chưa Liên Kết",
                description="Tài khoản Discord này chưa được liên kết với bất kỳ phòng trọ nào.\n\n"
                            "Vui lòng liên kết tài khoản bằng lệnh `/link` trước khi gửi tin nhắn liên hệ.",
                color=COLOR_DANGER
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        ...
        success = database.save_message_from_tenant(
            sender_username=sender_username,
            receiver_username="admin",
            message_content=noidung,
            message_type="text",
            discord_message_id=f"slash_msg_{interaction.id}"
        )
        if success:
            broadcast_saved_message(self.bot, sender_username, "admin", noidung, "text", f"slash_msg_{interaction.id}")
            embed = build_base_embed(
                title="✉️ Gửi Tin nhắn Thành công",
                description=f"Tin nhắn đã chuyển tiếp thành công đến chủ trọ:\n\n"
                            f"> *{noidung}*\n\n"
                            f"Chủ trọ sẽ nhận được thông báo trên phần mềm và phản hồi sớm nhất qua tin nhắn riêng của Bot này.",
                color=COLOR_SUCCESS
            )
